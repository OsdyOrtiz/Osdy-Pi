...
function managementError(result: { code: number; stderr: string }): string {
	return result.stderr.replaceAll(/[\r\n]+/g, " ").trim().slice(0, 180) || `command exited ${result.code}`;
}
...
async function runManagementCommand(
	...
	try {
		...
		ctx.ui.notify("Account operation failed before it could complete.", "warning");
		return undefined;
	}
}

function markedProfiles(profiles: string[], activeProfile: string | undefined, defaultProfile: string | undefined): string[] {
	return profiles.map((profile) => `${profile}${profile === activeProfile ? " (active)" : ""}${profile === defaultProfile ? " (default)" : ""}`);
}
...
export async function manageAccountProfile(ctx: AccountContext, mode: "rename" | "remove", dependencies: AccountManagementDependencies): Promise<void> {
	let profiles: string[];
	try { profiles = await dependencies.profiles(); } catch { ctx.ui.notify("Could not list account profiles.", "warning"); return; }
	if (profiles.length === 0) { ctx.ui.notify("No account profiles are available.", "warning"); return; }
	const selected = await ctx.ui.select(mode === "rename" ? "Rename OpenAI account" : "Remove OpenAI account permanently", markedProfiles(profiles, dependencies.activeProfile, undefined));
	if (selected === undefined) return;
	const activeMarker = selected.indexOf(" (");
	const target = activeMarker === -1 ? selected : selected.slice(0, activeMarker);
	if (!profiles.includes(target)) return;
	if (target === dependencies.activeProfile) { ctx.ui.notify("Cannot change the active profile. Switch to another profile first.", "warning"); return; }
	if (mode === "rename") {
		const newName = await ctx.ui.input("New profile name");
		if (newName === undefined) return;
		if (!validProfileName(newName)) { ctx.ui.notify("Profile names use lowercase letters, numbers, and hyphens only.", "warning"); return; }
		const result = await runManagementCommand(ctx, dependencies, ["account", "rename", target, newName]);
		if (result) ctx.ui.notify(result.code === 0 ? `Account profile renamed to ${newName}.` : `Rename failed: ${managementError(result)}`, result.code === 0 ? "info" : "warning");
		return;
	}
	...
	if (defaultState.status === "valid" && defaultState.profile === target) {
		replacement = await ctx.ui.select("Choose a new default account", profiles.filter((profile) => profile !== target));
		if (replacement === undefined) return;
	}
	const confirmation = await ctx.ui.input(`Type ${target} to permanently remove it`);
	if (confirmation === undefined) return;
	if (confirmation !== target) { ctx.ui.notify("Removal cancelled: confirmation did not match the profile name.", "warning"); return; }
	...
	if (result) ctx.ui.notify(result.code === 0 ? `Account profile ${target} permanently removed.` : `Removal failed: ${managementError(result)}`, result.code === 0 ? "info" : "warning");
}

export async function manageAccountProfiles(ctx: AccountContext, dependencies: AccountManagementDependencies): Promise<void> {
	for (;;) {
		let profiles: string[];
		try { profiles = await dependencies.profiles(); } catch { ctx.ui.notify("Could not list account profiles.", "warning"); return; }
		const defaultState = await currentDefault(dependencies);
		const defaultProfile = defaultState.status === "valid" ? defaultState.profile : undefined;
		const action = await ctx.ui.select("OpenAI account manager", ["Switch", "Add", "Default", "Rename", "Remove", "Account info", "Cancel"]);
		if (action === undefined || action === "Cancel") return;
		if (action === "Account info") {
			const defaultNote = defaultState.status === "invalid"
				? "\nDefault account metadata is invalid; clear or fix the default account."
				: defaultState.status === "error"
					? "\nDefault account unavailable."
					: "";
			ctx.ui.notify(`Accounts:\n${markedProfiles(profiles, dependencies.activeProfile, defaultProfile).join("\n") || "No account profiles."}${defaultNote}`, "info");
			continue;
		}
		if (action === "Add") {
			const name = await ctx.ui.input("New profile name");
			if (name === undefined) continue;
			if (!validProfileName(name)) { ctx.ui.notify("Profile names use lowercase letters, numbers, and hyphens only.", "warning"); continue; }
			const result = await runManagementCommand(ctx, dependencies, ["account", "create", name]);
			if (result) ctx.ui.notify(result.code === 0 ? "Account profile created. Select it later via Switch, then run /login and select ChatGPT Plus/Pro (Codex)." : `Create failed: ${managementError(result)}`, result.code === 0 ? "info" : "warning");
			continue;
		}
		if (action === "Default") {
			const choice = await ctx.ui.select("Default account", ["Show current", "Set default", "Clear default", "Back"]);
			if (choice === "Set default") { const profile = await ctx.ui.select("Choose default account", profiles); if (profile !== undefined) { const result = await runManagementCommand(ctx, dependencies, ["account", "default", profile]); if (result) ctx.ui.notify(result.code === 0 ? `Default account set to ${profile}.` : `Default failed: ${managementError(result)}`, result.code === 0 ? "info" : "warning"); } }
			else if (choice === "Clear default") { const result = await runManagementCommand(ctx, dependencies, ["account", "default", "--clear"]); if (result) ctx.ui.notify(result.code === 0 ? "Default account cleared." : `Default failed: ${managementError(result)}`, result.code === 0 ? "info" : "warning"); }
			else if (choice === "Show current") {
				if (defaultState.status === "valid") ctx.ui.notify(`Default account: ${defaultState.profile}`, "info");
				else if (defaultState.status === "unset") ctx.ui.notify("No default account.", "info");
				else if (defaultState.status === "invalid") ctx.ui.notify("Default account metadata is invalid; no account selected.", "warning");
				else ctx.ui.notify("Default account is unavailable; clear or fix the default account before continuing.", "warning");
			}
			continue;
		}
		if (action === "Rename" || action === "Remove") { await manageAccountProfile(ctx, action.toLowerCase() as "rename" | "remove", dependencies); continue; }
		if (profiles.length === 0) { ctx.ui.notify("No account profiles are available.", "warning"); continue; }
		const target = await ctx.ui.select("Switch OpenAI account", markedProfiles(profiles, dependencies.activeProfile, defaultProfile ?? undefined));
		if (target === undefined) continue;
		const marker = target.indexOf(" (");
		const profile = marker === -1 ? target : target.slice(0, marker);
		if (!profiles.includes(profile)) continue;
		if (profile === dependencies.activeProfile) { ctx.ui.notify("That account is already active.", "info"); continue; }
		if (dependencies.handoff) await dependencies.handoff(profile);
		return;
	}
}

function runBundledCommand(args: string[]): Promise<{ code: number; stdout: string; stderr: string }> {
	...
	pi.registerCommand("osdy-account", {
		...
		handler: async (args, ctx) => {
			...
			const dependencies: AccountManagementDependencies = {
				...
				handoff: (profile) => handoffToAccount(ctx, profile, {
					launcher: bundledLauncherPath(),
					spawn: (command, launchArgs) => waitForLauncherReady(spawnChild(process.execPath, [command, ...launchArgs], {
						detached: true, stdio: ["ignore", "ignore", "ignore", "ipc"],
					})),
				}),
			};
			if (mode === "rename" || mode === "remove") { await manageAccountProfile(ctx, mode, dependencies); return; }
			if (mode) { ctx.ui.notify("Usage: /osdy-account [rename|remove]", "warning"); return; }
